...
app.get('/', (request, response) => {
  response.send(html_header + '<h2><b>¿Que es el Package.json?</b></h2>' + 
  '<p>El archivo <b>package.json</b> es la configuración principal de un proyecto en Node.js. ' +
  'Contiene la información del proyecto, sus dependencias y los scripts que se pueden ejecutar.</p>' +
  '<p>En este laboratorio se manejo Express, Nodemon y manejar rutas y esto se tiene que configurar dentro del archivo.</p>' + 
  html_footer);
});

//Colocamos donde esta definida la ruta
const rutapersonajes = require('./routes/personajes.routes.js');
...
//Este middleware interfiere con la carga del HTML si se define antes
//se recomienda colocarlo al final para dejar que cargue primero la pagina
app.use((request, response, next) => {
